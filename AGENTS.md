...
| `hooks/grok-hook.js` / `hooks/grok-install.js` | Grok Build state + Notification command hook；写入 `~/.grok/hooks/clawd.json`；无 session id 时丢弃；不注册 `/permission` |
...
- Grok Build 通过 `~/.grok/hooks/clawd.json`（或 `$GROK_HOME/hooks/clawd.json`）的 Claude Code 兼容 command hooks 做 **state + Notification only** 集成：stdin 为 camelCase（`sessionId` / `toolName`），`agent_id` 必须是 `grok`。所有权以 `grok-hook.js` marker 为准，安装/卸载只合并或移除托管条目，不得整文件覆盖用户 hook。Windows 命令必须带 PowerShell `&` 调用运算符。Grok 没有 `PermissionRequest`，不注册 `/permission`。Grok 默认会扫 `~/.claude/settings.json`，所以 `clawd-hook.js` 和 `auto-start.js` 在 `GROK_HOOK_EVENT` / `GROK_SESSION_ID` 下必须直接退出。`Stop` 的 teardown / `stopHookActive` / 仍在跑的 background task 不得当成完成。
