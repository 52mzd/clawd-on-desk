...
- OMP (oh-my-pi) 通过 `~/.omp/agent/extensions/clawd-on-desk` 的 global extension 推送状态，agent id `omp`，对 Clawd 同样是 **state-only**。它是 Pi 的 fork（包名 `@oh-my-pi/pi-coding-agent`），所以 `hooks/omp-extension.ts` 刻意与 `hooks/pi-extension.ts` 保持逐行对齐（只差 package import、core import 与函数名），进程树遍历与终端识别不得各自漂移；OMP 独有的差异全部收在 `hooks/omp-extension-core.js`。**完成事件必须绑 `session_stop`，不得绑 `agent_end`** —— OMP 在每个 agent-loop 边界都会发 `agent_end`（含后台任务未结束、排队跟进、工具调用仍在飞的调度暂停），用它报完成会让 Clawd 在会话还在干活时就放完成音；`session_stop` 才是结算过的一轮。`session_switch` / `session_branch` 要为被切走的旧 session 补发一条合成 `SessionEnd`，否则 Clawd 会留下一条再也不会有人上报的活动行。session id 命名空间是 `omp:<raw>`，且必须始终带 `session_title`（多个交互式会话合法地共用同一个 cwd，缺标题会让列表和跳转目标无法区分）。安装器遇到社区桥接 `~/.omp/agent/extensions/clawd-on-desk-omp.ts` 时必须 fail closed 跳过：两者上报同一批事件，同时存在会双发。当前 `assets/source/agent-icons/omp.png` 是 `fallback: true` 的占位 π 字形，不是 OMP 官方品牌素材，拿到授权素材后应替换
