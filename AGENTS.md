...
| `hooks/minimax-hook.js` / `hooks/minimax-install.js` | MiniMax Code state-only 本地插件：整目录 install/uninstall（`<MINIMAX_DATA_DIR 或 ~/.minimax>/plugins/clawd-state/`），CLAUDE 格式 hooks + exec-form args，10 事件无 PermissionRequest/Notification，目录级所有权 fail closed |
