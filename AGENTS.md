...
| `hooks/minimax-hook.js` / `hooks/minimax-install.js` | MiniMax Code state-only 本地插件：整目录 install/uninstall（`<MINIMAX_DATA_DIR 或 MAVIS_DATA_DIR 或 ~/.minimax>/plugins/clawd-state/`），CLAUDE 格式 hooks + exec-form args，10 事件无 PermissionRequest/Notification，目录级所有权 fail closed |
...
- `agentCmdlineCheck` 只对进程名在 `agentCmdlineNames` 里的祖先进程运行，默认是 `DEFAULT_AGENT_CMDLINE_NAMES`：`node` / `node.exe`，加上 Linux 上没设过 `process.title` 的 Node 在 `ps -o comm=` 里的名字——Node 23.8–25.4（含全部 24.x）把主线程命名为 `MainThread`、≥ 25.5 为 `node-MainThread`，Linux 报的进程名就是主线程名。传入 `agentCmdlineNames` 会整体替换默认值：只想加名字的 adapter 必须在选项里直接写成 `agentCmdlineNames: new Set([..., ...DEFAULT_AGENT_CMDLINE_NAMES])`（如 ZCode；可以跨行，方括号里不要夹注释，展开必须是独立的数组元素，不能接 `.slice()` 之类；简写属性、先存进变量、在注释里提到它都会被判不合格，因为文本检查无法核对），只有刻意只探测非 Node 宿主的（WorkBuddy 的 `electron`）例外，`test/shared-process.test.js` 会扫描 `hooks/*.js` 检查
