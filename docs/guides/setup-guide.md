...
- **Ownership:** the whole `clawd-state/` plugin directory belongs to Clawd. Clawd proves that with an ownership marker file (`.clawd-managed.json`): a first install is built in a staging folder and moved into place in one step, install refuses to touch an existing directory without a valid marker, and uninstall removes the directory only when the marker is present and no managed path is a symlink. A directory name or a script name inside a file never counts as ownership, so a `clawd-state/` folder left by a pre-release build (no marker) must be deleted by hand before installing; if Clawd cannot remove the folder, Uninstall reports its path instead of claiming success.
