...
> **이 저장소는 fork입니다.** 최신 업스트림 [rullerzhou-afk/clawd-on-desk](https://github.com/rullerzhou-afk/clawd-on-desk)을 기반으로 합니다.
>
> **이 fork가 추가한 것 — Trellis 워크플로 통합:**
> - **Dashboard 작업 뷰** — 등록된 프로젝트 전반의 작업 탐색: 단계별 그룹, 진행률과 체크리스트, 작업 트리, 스펙 맵, 아카이브
> - **설정 → Trellis** — 프로젝트 루트 등록, `trellis update`의 실제 dry-run 미리보기가 포함된 설치/업그레이드 마법사
> - **펫 상태 피드백** — 병렬 작업 저글링, 계획 단계의 마법사 모자, 단계 전환 말풍선, 완료 축하
> - **HUD 작업 배지** — 각 라이브 세션에 연결된 Trellis 작업. 클릭하면 인라인 상세 행이 펼쳐집니다
>
> 자세한 문서는 `docs/project/trellis-settings-panel.md`에 있습니다. 라이선스는 업스트림과 동일한 AGPL-3.0입니다.
