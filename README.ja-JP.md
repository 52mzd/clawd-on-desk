...
> **これは fork です。** 最新の上流 [rullerzhou-afk/clawd-on-desk](https://github.com/rullerzhou-afk/clawd-on-desk) をベースにしています。
>
> **この fork で追加されたもの — Trellis ワークフロー連携:**
> - **Dashboard のタスクビュー** — 登録済みプロジェクトを横断してタスクを閲覧：フェーズ別グループ、進捗とチェックリスト、タスクツリー、スペックマップ、アーカイブ
> - **設定 → Trellis** — プロジェクトルートの登録、`trellis update` の実際の dry-run プレビュー付きインストール／アップグレードウィザード
> - **ペットの状態フィードバック** — 並列タスクのジャグリング、プランニング中のウィザードハット、フェーズ遷移バブル、完了時のお祝い
> - **HUD タスクバッジ** — 各ライブセッションに紐づく Trellis タスク。クリックでインライン詳細行を展開
>
> 詳細なドキュメントは `docs/project/trellis-settings-panel.md` にあります。ライセンスは上流と同じ AGPL-3.0 です。
