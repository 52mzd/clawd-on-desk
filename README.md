...
- **MiniMax Code** — experimental, state-only integration through a Clawd-owned local plugin at `~/.minimax/plugins/clawd-state/` (install from Settings → Agents or run `npm run install:minimax-hooks`, uninstall with `npm run uninstall:minimax-hooks`; honors `MINIMAX_DATA_DIR`). Covers the mcode CLI and the MiniMax Code desktop app. Clawd registers 10 state events (including `SessionEnd`); `PermissionRequest` is deliberately not registered because MiniMax's plugin-hook runner caps every handler at 1–10 seconds, so blocking human approval is impossible — every approval stays in MiniMax's native flow and the hook stdout is always `{}`. On macOS the desktop app auto-discovers the plugin; if hooks do not fire, enable it with `mcode plugin enable clawd-state@local` or via the app's plugin panel. Session titles derive from the first prompt line and keep the first one per session
