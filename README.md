...
- **Grok Build** — optional Claude Code-compatible command hooks via `~/.grok/hooks/clawd.json` (install from Settings → Agents or run `node hooks/grok-install.js`). State + Notification only: Grok has no blocking `PermissionRequest` hook, so Allow/Deny stays in the Grok TUI.
