...
- **Grok Build** — 可选 Claude Code 兼容 command hook，写入 `~/.grok/hooks/clawd.json`（从 Settings → Agents 安装，或执行 `node hooks/grok-install.js`）。仅状态 + 通知：Grok 没有阻塞式 `PermissionRequest`，允许/拒绝仍在 Grok 终端里完成
